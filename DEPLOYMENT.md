# QGR Exchange — Production Deployment Guide

> Step-by-step instructions for deploying QGR Exchange to Vercel (frontend) and shared hosting (backend), including domain configuration.
...
- A **domain name** (e.g., `quantumgesararedemption.com`) — from Namecheap, GoDaddy, Cloudflare, etc.
...
3. Then configure your hosting to point a **subdomain** (e.g., `api.quantumgesararedemption.com`) to `~/banking-api/public/`.

> **Alternatively:** If you can't configure subdomains freely, you can upload `banking-api/` inside `public_html/` as a subfolder: `public_html/banking-api/`. Then your API URL becomes `https://quantumgesararedemption.com/banking-api/public/api`.
...
APP_URL=https://api.quantumgesararedemption.com
```
...
2. Create subdomain: `api.quantumgesararedemption.com`
...
Visit `https://api.quantumgesararedemption.com/index.php` in your browser. You should see a JSON response:

```json
{ "status": "QGR Exchange API running" }
```
...
| Variable | Value |
|---|---|
| `VITE_API_URL` | `https://api.quantumgesararedemption.com/api` |
...
Since QGR Exchange is a SPA, refreshing `/dashboard` or any deep link will 404 without a redirect rule.
...
2. Click **Add Domain**.
3. Type your domain: `quantumgesararedemption.com`.
...
3. Add an A record for `api.quantumgesararedemption.com` pointing to your hosting server's IP (get this from your hosting panel).
...
| Type | Name | Value |
|---|---|---|
| `A` | `@` (or `quantumgesararedemption.com`) | `76.76.21.21` (Vercel IP) |
...
```bash
dig quantumgesararedemption.com
nslookup quantumgesararedemption.com 8.8.8.8
```

### Step 4: SSL Certificates

- **Vercel** automatically provisions and renews SSL for `quantumgesararedemption.com` via Let's Encrypt. No action needed.
- **Shared hosting** (`api.quantumgesararedemption.com`) — Enable Let's Encrypt SSL in your hosting panel's SSL/TLS section. Most hosts (Hostinger, Bluehost) do this with one click.
...
```env
CORS_ORIGIN=https://quantumgesararedemption.com
```
...
- [ ] Test API health: `curl https://api.quantumgesararedemption.com/` returns JSON
...
```sql
UPDATE users SET role = 'admin' WHERE email = 'admin@quantumgesararedemption.com';
```
