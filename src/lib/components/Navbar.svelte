    ...
    <div class="flex h-16 justify-between">
      ...
      <div class="hidden items-center gap-6 md:flex">
        {#if $auth.isAuthenticated}
          <a href="/dashboard" class="font-display text-xs font-bold tracking-widest hover:bg-acid hover:px-1">OVERVIEW</a>
          <a href="/dashboard/deposit" class="font-display text-xs font-bold tracking-widest hover:bg-acid hover:px-1">DEPOSIT</a>
          <a href="/dashboard/wallets" class="font-display text-xs font-bold tracking-widest hover:bg-acid hover:px-1">WALLETS</a>
          ...
          <div class="flex items-center gap-3 border-l-2 border-ink pl-4">
            <div class="text-right min-w-0">
              <p class="font-display truncate text-sm font-bold">{ $auth.user?.first_name } { $auth.user?.last_name }</p>
              <p class="font-mono max-w-48 truncate text-xs text-ink/50">{ $auth.user?.email }</p>
            </div>
            <button onclick={handleLogout} class="border-2 border-ink bg-white px-3 py-1.5 font-display text-xs font-bold hover:bg-blood hover:text-white">LOG OUT</button>
          </div>
        {:else}
          <Translate />
          <a href="/login" class="font-display text-xs font-bold tracking-widest hover:bg-acid hover:px-1">LOG IN</a>
          <a href="/signup" class="btn-primary !px-4 !py-2 !text-xs">INVEST →</a>
        {/if}
      </div>
  ...
  {#if mobileMenuOpen}
    <div class="border-t-2 border-ink bg-paper md:hidden">
      <div class="space-y-1 px-4 py-4">
        {#if $auth.isAuthenticated}
          <a href="/dashboard" class="block py-2 font-display text-sm font-bold tracking-widest">OVERVIEW</a>
          <a href="/dashboard/deposit" class="block py-2 font-display text-sm font-bold tracking-widest">DEPOSIT</a>
          <a href="/dashboard/wallets" class="block py-2 font-display text-sm font-bold tracking-widest">WALLETS</a>
          <a href="/dashboard/settings" class="block py-2 font-display text-sm font-bold tracking-widest">CONFIG</a>
          <button onclick={handleLogout} class="block w-full py-2 text-left font-display text-sm font-bold text-blood">LOG OUT</button>
        {:else}
          <a href="/login" class="block py-2 font-display text-sm font-bold tracking-widest">LOG IN</a>
          <a href="/signup" class="btn-primary mt-2 block w-full text-center">INVEST →</a>
        {/if}
      </div>
    </div>
  {/if}
