<script lang="ts">
  ...
  async function adjust() {
    ...
    if (res.success) {
      ...
      toast.success('Balance updated. Audit trail written.');
      await loadWallets();
    } else {
...
<div class="space-y-6">
  <div>
    ...
    <h1 class="font-display mt-1 text-3xl font-bold tracking-tight">BALANCE OPERATIONS (AUDITED)</h1>
    <p class="mt-1 font-mono text-xs text-ink/60">EVERY ACTION IS WRITTEN TO THE AUDIT TRAIL.</p>
  </div>

  {#if msg}<div class="brut-flat border-ink bg-acid p-4 text-sm font-bold">{msg}</div>{/if}
  {#if msgErr}<div class="brut-flat border-blood bg-white p-4 text-sm font-bold text-blood">ERROR: {msgErr}</div>{/if}
  ...
  <section class="brut-card bg-white p-5">
    ...
    {#if !pending.length}
      <p class="border-2 border-dashed border-ink/30 py-8 text-center font-mono text-sm text-ink/50">QUEUE CLEAR. NOTHING PENDING REVIEW.</p>
    {:else}
  ...
  <section class="brut-card bg-white p-5">
    <h2 class="font-display text-lg font-bold">BALANCE ADJUSTMENT</h2>
  ...
  <section class="brut-card bg-white p-5">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h2 class="font-display text-lg font-bold">WALLET REGISTRY</h2>
        <p class="mt-0.5 font-mono text-xs text-ink/50">EVERY WALLET, EVERY INVESTOR</p>
      </div>
      <div class="flex flex-wrap gap-2">
        <input bind:value={search} placeholder="email / address / symbol…" class="input-base max-w-xs flex-1 font-mono text-xs" />
        <button class="btn-secondary !px-3 !py-2 !text-xs" onclick={loadWallets}>SEARCH</button>
      </div>
    </div>
    {#if loading}
      <div class="flex justify-center py-10"><LoadingSpinner /></div>
    {:else if !wallets.length}
      <p class="py-10 text-center font-mono text-sm text-ink/50">NO WALLETS FOUND. ADJUST THE SEARCH AND TRY AGAIN.</p>
    {:else}
      <div class="mt-4 overflow-x-auto">
        <table class="brut-table w-full min-w-[760px]">
          <thead>
            <tr><th>INVESTOR</th><th>WALLET</th><th>ADDRESS</th><th class="text-right">BALANCE</th><th class="text-right">SPENDABLE</th><th class="text-right">VALUE</th><th>STATUS</th></tr>
          </thead>
          <tbody class="font-mono text-xs">
            {#each wallets as w}
              <tr>
                ...
                <td class="break-all">{w.address.slice(0, 12)}…{w.address.slice(-6)}</td>
