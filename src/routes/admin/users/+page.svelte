...
<svelte:head><title>User Management — QGR Exchange Admin</title></svelte:head>
...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    ...
    <button onclick={() => showCreate = true}
      class="flex items-center gap-2  bg-gold px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-gold-deep transition-colors">
      <span class="text-base">➕</span> Create User
    </button>
  </div>

  <!-- Filters -->
  <div class="flex flex-col gap-3 sm:flex-row  border border-slate-100 bg-white p-4 shadow-sm">
    <input type="text" bind:value={searchQuery} placeholder="Search by name or email…"
      class="flex-1  border border-slate-200 px-4 py-2.5 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none" />
    <select bind:value={statusFilter}
      class=" border border-slate-200 px-4 py-2.5 text-sm focus:border-gold outline-none">
    ...
    <button onclick={loadUsers} class=" border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">
      🔄 Refresh
    </button>
  </div>

  <!-- Table -->
  <div class="overflow-hidden  border border-slate-100 bg-white shadow-sm">
    {#if loading}
              ...
              <tr class="hover:bg-slate-50 transition-colors">
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    <div class="flex h-9 w-9 flex-shrink-0 items-center justify-center  bg-gradient-to-br from-gold to-gold text-xs font-bold text-white">
                ...
                <td class="px-5 py-4">
                  <span class=" px-2.5 py-1 text-xs font-semibold capitalize {user.role === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-700'}">{user.role}</span>
                </td>
                <td class="px-5 py-4">
                  <span class=" px-2.5 py-1 text-xs font-semibold capitalize {statusColor(user.status)}">{user.status}</span>
                </td>
                <td class="px-5 py-4 text-xs text-slate-400">{formatDate(user.created_at)}</td>
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    <a href="/admin/users/{user.id}" class="text-xs font-semibold text-gold hover:underline">Manage</a>
                    <button onclick={() => handleToggleStatus(user)} class="text-xs font-semibold text-amber-600 hover:underline">
                      {user.status === 'active' ? 'Suspend' : 'Activate'}
                    </button>
                    <button onclick={() => handleDelete(user)} class="text-xs font-semibold text-red-500 hover:underline">Delete</button>
                  </div>
                </td>
              </tr>
            {/each}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
    <div class="w-full max-w-lg  bg-white shadow-2xl overflow-hidden">
      ...
      <div class="flex items-center justify-between border-b border-slate-100 px-6 py-5">
        ...
        <button onclick={() => { showCreate = false; createErrors = []; }} class="text-slate-400 hover:text-slate-600 text-2xl font-light">×</button>
      </div>
      ...
      <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" bind:value={newUser.first_name} class="block w-full  border border-slate-200 px-4 py-2.5 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none" placeholder="John"/>
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Last Name *</label>
            <input type="text" bind:value={newUser.last_name} class="block w-full  border border-slate-200 px-4 py-2.5 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none" placeholder="Doe"/>
          </div>
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-600 mb-1.5">Email Address *</label>
          <input type="email" bind:value={newUser.email} class="block w-full  border border-slate-200 px-4 py-2.5 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none" placeholder="john@example.com"/>
        </div>
        ...
        <div>
          ...
          <input type="password" bind:value={newUser.password} class="block w-full  border border-slate-200 px-4 py-2.5 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none" placeholder="••••••••"/>
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-600 mb-1.5">Phone</label>
          <input type="tel" bind:value={newUser.phone} class="block w-full  border border-slate-200 px-4 py-2.5 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none" placeholder="(555) 123-4567"/>
        </div>

        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Role</label>
            <select bind:value={newUser.role} class="block w-full  border border-slate-200 px-4 py-2.5 text-sm focus:border-gold outline-none">
          ...
          <div>
            ...
            <select bind:value={newUser.status} class="block w-full  border border-slate-200 px-4 py-2.5 text-sm focus:border-gold outline-none">
              <option value="active">Active</option>
              <option value="pending">Pending</option>
              <option value="suspended">Suspended</option>
            </select>
        ...
        <div>
          ...
          <div class="relative">
            ...
            <input type="number" bind:value={newUser.initial_balance} step="0.01" min="0"
              class="block w-full  border border-slate-200 pl-8 pr-4 py-2.5 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none" placeholder="0.00"/>
          </div>
      ...
      <div class="flex gap-3 border-t border-slate-100 px-6 py-5">
        <button onclick={() => { showCreate = false; createErrors = []; }}
          class="flex-1  border border-slate-200 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
          Cancel
        </button>
        <button onclick={handleCreate} disabled={saving}
          class="flex-1  bg-gold py-3 text-sm font-semibold text-white hover:bg-gold-deep disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
