...
<svelte:head>
  <title>Card Management — QGR Exchange Admin</title>
</svelte:head>
...
<div class="space-y-6">
  ...
  <!-- Filters -->
  <div class="flex flex-wrap gap-3  border border-slate-100 bg-white p-4 shadow-sm">
    <input
      type="text"
      bind:value={searchQ}
      placeholder="Search by name, email, card number…"
      class="min-w-[200px] flex-1  border border-slate-200 px-4 py-2.5 text-sm focus:border-red-500 focus:ring-2 focus:ring-red-500/20 focus:outline-none"
    />
    <select
      bind:value={statusFilter}
      class=" border border-slate-200 px-4 py-2.5 text-sm focus:border-red-500 focus:outline-none"
    >
                ...
                <td class="px-5 py-4">
                  {#if card.status !== 'reported_lost'}
                    <button
                      ...
                      class="text-xs font-semibold {card.status === 'active' ? 'text-amber-600 hover:underline' : 'text-gold hover:underline'} disabled:opacity-50"
                    >
