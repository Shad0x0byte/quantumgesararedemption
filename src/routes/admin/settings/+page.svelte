<script lang="ts">
  ...
  // These values are informational; the real config lives in backend .env.
  ...
  const envConfig = [
    { key: 'API URL',         value: import.meta.env.VITE_API_URL || 'http://localhost:8002/api' },
...
<div class="space-y-8">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    ...
    <div class=" border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {#if pwErrors.length}
        <div class="mb-4  border border-red-200 bg-red-50 p-4">
      ...
      <div class="space-y-4">
        <div>
          ...
          <input type="password" bind:value={pwData.current}
            class="block w-full  border border-slate-200 px-4 py-3 text-sm focus:border-red-500 focus:ring-2 focus:ring-red-500/20 focus:outline-none" />
        </div>
        <div>
        ...
        <button
          ...
          disabled={pwLoading}
          class="flex w-full items-center justify-center gap-2  bg-slate-900 py-3 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50 transition-colors"
        >
