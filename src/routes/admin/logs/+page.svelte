          ...
          <tbody class="divide-y divide-slate-50 text-sm">
            {#each logs as log}
              <tr class="hover:bg-slate-50">
                ...
                <td class="px-5 py-3.5">
                  <span class=" bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">
                    {log.action}
                  </span>
                </td>
          ...
          <tbody class="divide-y divide-slate-50 text-sm">
            {#each logs as log}
              <tr class="hover:bg-slate-50">
                ...
                <td class="px-5 py-3.5">
                  <span class=" bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700 capitalize">
                ...
                <td class="px-5 py-3.5">
                  <span class=" px-2.5 py-1 text-xs font-semibold capitalize {riskColor(log.risk_level)}">
                    {log.risk_level}
                  </span>
                </td>
                <td class="px-5 py-3.5 text-slate-400 text-xs whitespace-nowrap">{formatDate(log.created_at)}</td>
  ...
  <div class="flex items-center justify-between text-sm text-slate-500">
    ...
    <div class="flex gap-2">
      <button
        ...
        disabled={page <= 1}
        class=" border border-slate-200 px-4 py-2 hover:bg-slate-50 disabled:opacity-40 transition-colors"
      >← Prev</button>
      <button
        onclick={() => { page++; fetchLogs(); }}
