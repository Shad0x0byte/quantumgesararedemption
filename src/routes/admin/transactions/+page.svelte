<script lang="ts">
	...
	function getStatusColor(status: string): string {
		switch (status) {
			case 'completed': return 'bg-paper-dim text-gold-deep';
...
<svelte:head><title>All Transactions - QGR Exchange Admin</title></svelte:head>
...
<div>
	...
	<!-- Filters -->
	<div class="mb-6  border border-slate-200 bg-white p-4 shadow-sm">
		<div class="flex flex-col gap-4 lg:flex-row">
			<div class="flex-1">
				<input type="text" bind:value={searchQuery} placeholder="Search by description, merchant or user ID..." class="block w-full  border border-slate-200 bg-white px-4 py-3 focus:border-gold focus:ring-2 focus:ring-gold/20 sm:text-sm" />
			</div>
			<div class="flex gap-2">
				<select bind:value={typeFilter} class=" border border-slate-200 bg-white px-4 py-3 sm:text-sm">
				...
				<select bind:value={statusFilter} class=" border border-slate-200 bg-white px-4 py-3 sm:text-sm">
				...
				<tbody class="divide-y divide-slate-100">
					{#each filteredTransactions as tx}
						<tr class="transition-colors hover:bg-slate-50">
							...
							<td class="px-6 py-4">
								<span class="inline-flex items-center  bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700 capitalize">{tx.type}</span>
							</td>
							<td class="px-6 py-4 text-sm text-slate-500">{formatDate(tx.created_at || tx.date)}</td>
							<td class="px-6 py-4">
								<span class="inline-flex items-center  px-2.5 py-1 text-xs font-semibold capitalize {getStatusColor(tx.status)}">{tx.status}</span>
							</td>
