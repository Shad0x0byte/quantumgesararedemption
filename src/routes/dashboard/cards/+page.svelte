...
<svelte:head>
  <title>Cards - QGR Exchange</title>
</svelte:head>
...
<div class="space-y-8">
  ...
  {#if user?.status === 'suspended'}
    <div class="flex flex-col gap-4  border border-red-200 bg-red-50 p-5 shadow-sm sm:flex-row sm:items-center">
  ...
  {:else if cards.length === 0}
    <div class=" border border-slate-100 bg-white p-12 text-center shadow-sm">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    {#each cards as card}
      <div class="space-y-4">
        ...
        <!-- Visual Card -->
        <div class="relative  bg-gradient-to-br {cardBg(card.card_network)} p-6 text-white shadow-2xl min-h-[200px] overflow-hidden
                    {card.status === 'locked' ? 'opacity-70' : ''}">
          <!-- Background pattern -->
          <div class="absolute -right-8 -top-8 h-40 w-40  bg-white/5"></div>
          <div class="absolute -left-4 -bottom-8 h-32 w-32  bg-white/5"></div>
          ...
          <div class="flex items-start justify-between mb-6">
            ...
            <div class="text-right">
              ...
              {#if card.status !== 'active'}
                <span class="mt-1 inline-block  bg-red-500/80 px-2 py-0.5 text-xs font-bold">
        ...
        <!-- Card Info Panel -->
        <div class=" border border-slate-100 bg-white p-5 shadow-sm space-y-4">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class=" px-2.5 py-1 text-xs font-semibold capitalize {statusColor(card.status)}">
              {card.status}
            </span>
          </div>
          ...
          <div class="flex items-center gap-3  bg-paper border border-paper-dim p-3">
            <span class="text-lg">🏦</span>
            <div>
              <p class="text-xs text-gold font-medium">Linked Account</p>
          ...
          <button
            ...
            class="flex w-full items-center justify-center gap-2  border py-3 text-sm font-semibold transition-all
              {card.status === 'active'
                ? 'border-amber-200 text-amber-700 hover:bg-amber-50'
                : 'border-emerald-200 text-gold-deep hover:bg-paper'}
              disabled:opacity-50"
          >
          ...
          {#if card.status !== 'reported_lost'}
          <button
            onclick={() => toast.info('Please call support@quantumgesararedemption.com to report a lost card.')}
            class="w-full  border border-red-200 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors"
          >
            🚨 Report Lost / Stolen
          </button>
          {/if}
  ...
  <!-- Security info -->
  <div class=" border border-slate-100 bg-white p-6 shadow-sm">
