...
<svelte:head>
  <title>Accounts - QGR Exchange</title>
</svelte:head>
...
<div class="space-y-8">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <a
      ...
      class="flex items-center gap-2  bg-gold px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-gold-deep transition-colors w-fit"
    >
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    {#each accounts as account}
      <div class=" border border-slate-100 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">

        <div class="flex items-start justify-between mb-4">
          <div class="flex h-14 w-14 items-center justify-center  bg-slate-100 text-3xl">
            {getAccountIcon(account.account_type)}
          </div>
          <span class="inline-flex items-center  px-2.5 py-0.5 text-xs font-medium capitalize {getStatusClass(account.status)}">
            {account.status}
          </span>
        </div>
        ...
        <div class="space-y-2 mb-6 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium text-gold">{getAPY(account.account_type)}</span>
          </div>
                </div>

        <a
          href="/dashboard/transfer"
          class="block w-full text-center  bg-gold px-4 py-2.5 text-sm font-semibold text-white hover:bg-gold-deep transition-colors"
        >
          Transfer
        </a>
      </div>
        ...
        <tbody class="divide-y divide-slate-50">
          {#each accounts as acc}
            <tr class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-6 py-4"><span class=" px-2.5 py-0.5 text-xs font-medium capitalize {getStatusClass(acc.status)}">{acc.status}</span></td>
