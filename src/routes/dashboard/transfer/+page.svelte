...
const US_BANKS = [
  ...
  { name: 'QGR Exchange', routing: '021000021' },
  { name: 'Other', routing: '' },
];
  ...
  async function handleAccountNumberChange() {
    ...
    if (transferData.selectedBank === 'QGR Exchange') {
      ...
      try {
        ...
        if (res.success && res.data?.found && res.data?.is_quantumgesararedemption) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const isQGRTransfer = data.selectedBank === 'QGR Exchange' && recipientAccount;

      if (data.transferType === 'internal' || isQGRTransfer) {
        const toAccountId = isQGRTransfer ? recipientAccount!.id : parseInt(data.toAccount);
      ...
      if (res.success) {
        ...
        if (isQGRTransfer && recipientAccount) {
...
<svelte:head><title>Transfer — QGR Exchange</title></svelte:head>
...
<div class="fade-in">
  ...
  {#if user?.status === 'suspended'}
    <div class="mb-6 flex flex-col gap-4  border border-red-200 bg-red-50 p-5 shadow-sm sm:flex-row sm:items-center">
        ...
        <div class="space-y-5 p-5 sm:p-6">
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-2">
              {#each [['internal','↔ Internal'],['external','🏦 External'],['wire','⚡ Wire']] as [val, label]}
                <button type="button" onclick={() => transferData.transferType = val as any}
                  class=" border py-2.5 text-xs font-semibold transition-all sm:text-sm
          ...
          <div>
            ...
            <select bind:value={transferData.fromAccount}
              class="block w-full  border border-slate-200 bg-white px-4 py-3 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none">
          ...
          {:else}
            ...
            <div>
              ...
              <select bind:value={transferData.selectedBank} onchange={handleBankChange}
                class="block w-full  border border-slate-200 bg-white px-4 py-3 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <input type="text" bind:value={transferData.toAccountExternal} oninput={handleAccountNumberChange} placeholder="1234567890"
                  class="block w-full  border border-slate-200 bg-white px-4 py-3 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none" />
                {#if lookingUpAccount}
                  <span class="mt-1 text-xs text-slate-500">Looking up account...</span>
                {:else if recipientAccount}
                  <span class="mt-1 text-xs text-gold font-medium">✓ Found: {recipientAccount.recipient_name} ({recipientAccount.account_type})</span>
                {/if}
              </div>
              <div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input type="number" bind:value={transferData.amount} placeholder="0.00" step="0.01" min="0.01"
                class="block w-full  border border-slate-200 bg-white py-3.5 pr-4 pl-10 text-xl font-bold text-slate-900 focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none" />
            </div>
            {#if fromAccount}
          ...
          <div>
            ...
            <textarea bind:value={transferData.note} placeholder="What's this for?" rows="2"
              class="block w-full resize-none  border border-slate-200 bg-white px-4 py-3 text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 focus:outline-none"></textarea>
          </div>

          {#if user?.status === 'suspended'}
            <a href="mailto:support@quantumgesararedemption.com"
               class="flex w-full items-center justify-center  bg-slate-100 px-4 py-4 text-base font-bold text-slate-500 transition-all hover:bg-slate-200">
              Contact Support to Transfer
            </a>
          {:else}
            <button type="button" onclick={handleReview}
              class="w-full  bg-gold px-4 py-4 text-base font-bold text-white shadow-sm transition-all hover:bg-gold-deep active:scale-[0.98]">
              Review Transfer →
            </button>
          {/if}
        </div>
    ...
    <div class="space-y-4">
      <div class=" border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <ul class="space-y-2.5 text-sm">
          <li class="flex items-start gap-2"><span class="font-bold text-gold mt-0.5">●</span><div><span class="font-semibold text-slate-900">Internal</span><p class="text-xs text-slate-500">Instant</p></div></li>
      ...
      <div class=" border border-slate-100 bg-slate-50 p-5">
...
{#if showConfirmation && confirmationData}
  <div class="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 backdrop-blur-sm sm:items-center sm:p-4">
    <div class="w-full max-w-md rounded-t-3xl bg-white p-6 shadow-2xl sm: sm:p-8">
      <div class="mb-5 text-center">
        <div class="mx-auto mb-3 flex h-16 w-16 items-center justify-center  bg-paper-dim">
      ...
      <div class="mb-5 space-y-0  bg-slate-50 overflow-hidden">
        ...
        <div class="flex justify-between border-b border-slate-100 px-4 py-3">
          ...
          <span class="text-sm font-semibold text-right max-w-[55%]">
            ...
            {:else if confirmationData.selectedBank === 'QGR Exchange' && recipientAccount}
              {recipientAccount.recipient_name} (QGR Exchange - {recipientAccount.account_type})
            {:else}
        ...
        <div class="flex justify-between border-b border-slate-100 px-4 py-3">
          ...
          <span class="text-xl font-bold text-gold">{formatCurrency(parseFloat(confirmationData.amount))}</span>
        </div>
      ...
      <div class="mb-5">
        ...
        <div class="relative">
          <input
            ...
            class="block w-full  border px-4 py-3.5 text-center text-2xl font-bold tracking-[0.5em] transition-all focus:outline-none
            {pinError ? 'border-red-400 bg-red-50 text-red-700 focus:ring-2 focus:ring-red-500/20' : 'border-slate-200 bg-white focus:border-gold focus:ring-2 focus:ring-gold/20'}"
          />
          <button type="button" onclick={() => showPin = !showPin}
      ...
      <div class="flex gap-3">
        <button onclick={() => (showConfirmation = false)}
          class="flex-1  border border-slate-200 px-4 py-3.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
          Cancel
        </button>
        <button onclick={handleConfirm} disabled={isLoading || pin.length !== 4}
          class="flex-1 flex items-center justify-center gap-2  bg-gold px-4 py-3.5 text-sm font-bold text-white shadow-sm transition-all hover:bg-gold-deep disabled:opacity-50 disabled:cursor-not-allowed">
          {#if isLoading}
...
{#if showSuccessModal && lastTransferResult}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
    <div class="w-full max-w-md animate-in fade-in zoom-in duration-300">
      <div class="overflow-hidden  bg-white shadow-2xl border border-slate-100">
        <!-- Receipt Header -->
        <div class="bg-gold px-6 py-8 text-center text-white relative">
          <div class="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none">
            <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
              <path d="M0 100 C 20 0 50 0 100 100 Z" fill="white"></path>
            </svg>
          </div>
          <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center  bg-white/20 backdrop-blur-md border border-white/30 shadow-inner">
            <span class="text-3xl">🏦</span>
          </div>
          <h2 class="text-xs font-bold uppercase tracking-widest opacity-80 mb-1">QGR Exchange</h2>
          <h3 class="font-display text-2xl font-bold">Transfer Successful</h3>
          <p class="mt-2 text-sm text-paper/80">Transaction Reference: <span class="font-mono font-bold">{lastTransferResult.reference_number}</span></p>
        </div>
        ...
        <div class="p-6 sm:p-8">
          ...
          <div class="space-y-4  bg-slate-50 p-5 border border-slate-100">
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="text-sm font-mono font-bold text-gold uppercase tracking-wider">{lastTransferResult.reference_number}</span>
            </div>
          </div>
          ...
          <div class="mt-8 flex flex-col gap-3">
            <button onclick={() => {
              ...
              class="flex w-full items-center justify-center gap-2  bg-slate-900 px-4 py-3.5 text-sm font-bold text-white shadow-lg shadow-slate-200 hover:bg-slate-800 transition-all active:scale-[0.98]">
              📄 View Full Receipt
            </button>
            <button onclick={closeSuccessModal}
              class="flex w-full items-center justify-center  border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">
              Done
            </button>
          </div>
        </div>
