...
<div>
	...
	<!-- Security Score -->
	<div class="mb-8  bg-gradient-to-br from-gold to-gold p-8 text-white shadow-xl shadow-gold/20">
	...
	<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
		<div class="space-y-8 lg:col-span-2">
			...
			<section class="overflow-hidden  border border-slate-200 bg-white shadow-sm">
				<div class="flex items-center justify-between border-b border-slate-200 p-6">
					...
					<button onclick={() => showPasswordModal = true} class=" bg-gold px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-gold">
						Change Password
					</button>
				</div>
				<div class="p-6">
					<div class="flex items-center gap-4  border border-paper-dim bg-paper p-4">
						<span class="text-2xl">✅</span>
						<div>
							<p class="font-semibold text-emerald-900">Strong Password</p>
							<p class="text-sm text-gold-deep">Your password meets all security requirements</p>
						</div>
					</div>
				</div>
			</section>
				...
				<div class="border-b border-slate-200 p-6">
					<div class="flex items-center justify-between">
						...
						<label class="relative inline-flex cursor-pointer items-center">
							...
							<div class="peer h-7 w-14  bg-slate-200 peer-checked:bg-gold peer-focus:ring-4 peer-focus:ring-emerald-300 peer-focus:outline-none after:absolute after:top-0.5 after:left-[4px] after:h-6 after:w-6 after: after:border after:border-slate-300 after:bg-white after:transition-all after:content-[''] peer-checked:after:translate-x-full peer-checked:after:border-white"></div>
						</label>
					</div>
				</div>
				<div class="p-6 space-y-3">
					<div class="flex items-center gap-4  bg-slate-50 p-4">
						<div class="flex h-12 w-12 items-center justify-center  bg-paper-dim text-2xl">📱</div>
						<div class="flex-1">
						...
						{#if twoFactorEnabled}
							<span class="inline-flex items-center  bg-paper-dim px-3 py-1 text-xs font-semibold text-gold-deep">✓ Active</span>
						{/if}
					</div>
				</div>
			</section>
		...
		<div class="space-y-8">
						...
						{#each sessions as session}
							<div class="p-4 transition-colors hover:bg-slate-50">
								<div class="mb-2 flex items-center justify-between">
									...
									{#if session.current}
										<span class="inline-flex items-center  bg-paper-dim px-2.5 py-1 text-xs font-semibold text-gold-deep">Current</span>
									{/if}
								</div>
			...
			<!-- Security Tips -->
			<section class="overflow-hidden  border border-blue-100 bg-gradient-to-br from-blue-50 to-purple-50 shadow-sm">
				<div class="p-6">
		...
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
			<div class="w-full max-w-md  bg-white p-8 shadow-2xl">
				<div class="mb-6 text-center">
					<div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center  bg-paper-dim"><span class="text-3xl">🔑</span></div>
				...
				<div class="mb-6 space-y-4">
					<div><label class="mb-2 block text-sm font-semibold text-slate-700">Current Password</label><input type="password" bind:value={passwordData.currentPassword} class="block w-full  border border-slate-200 bg-white px-4 py-3 sm:text-sm" /></div>
					<div><label class="mb-2 block text-sm font-semibold text-slate-700">New Password</label><input type="password" bind:value={passwordData.newPassword} class="block w-full  border border-slate-200 bg-white px-4 py-3 sm:text-sm" /></div>
					<div><label class="mb-2 block text-sm font-semibold text-slate-700">Confirm New Password</label><input type="password" bind:value={passwordData.confirmPassword} class="block w-full  border border-slate-200 bg-white px-4 py-3 sm:text-sm" /></div>
				</div>
				<div class="flex gap-3">
					<button onclick={() => { showPasswordModal = false; passwordData = { currentPassword: '', newPassword: '', confirmPassword: '' }; }} class="flex-1  border border-slate-300 px-4 py-3.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Cancel</button>
					<button onclick={handleSavePassword} class="flex-1  bg-gold px-4 py-3.5 text-sm font-bold text-white hover:bg-gold-deep">Save Password</button>
				</div>
			</div>
		</div>
	{/if}
