...
<svelte:head>
	<title>Statements - QGR Exchange</title>
</svelte:head>
		...
		<div class="flex items-center justify-between">
			...
			<div class="flex items-center gap-3">
				<select
					...
					class=" border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 focus:border-gold focus:ring-2 focus:ring-gold/20"
				>
					{#each years as year}
...
<div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-3">
  <div
  class=" bg-gradient-to-br from-gold to-gold p-6 text-white shadow-lg"
  >
  <p class="mb-2 text-sm font-medium text-paper-dim">Total Statements</p>
  <p class="text-4xl font-bold">{statements.length}</p>
  <p class="mt-2 text-sm text-paper-dim">Available for download</p>
  </div>
...
<div class="overflow-hidden  border border-slate-200 bg-white shadow-sm">
  ...
  {#each statements.filter(s => s.year.toString() === selectedYear) as statement}
    ...
    <div class="flex items-center justify-between">
      ...
      <div
        class="flex h-14 w-14 items-center justify-center  bg-slate-100 text-2xl"
      >
        📄
      </div>
      <div>
      ...
      <div class="flex items-center gap-3">
      <span
        class="inline-flex items-center  bg-paper-dim px-3 py-1 text-xs font-semibold text-gold-deep"
      >
        ✓ Ready
      </span>
      <button
        onclick={() => handleDownload(statement)}
        class="inline-flex transform items-center gap-2  bg-gold px-5 py-2.5 text-sm font-semibold text-white transition-all hover:scale-105 hover:bg-gold"
      >
        <span>⬇️</span>
        <span>Download</span>
      </button>
      <button
        onclick={() => handleEmail(statement)}
        class="inline-flex items-center gap-2  bg-slate-100 px-5 py-2.5 text-sm font-semibold text-slate-700 transition-all hover:bg-slate-200"
      >
        <span>📧</span>
        <span>Email</span>
      </button>
  ...
  {/if}
	...
	<!-- Statement Settings -->
	<div class="mt-8 overflow-hidden  border border-slate-200 bg-white shadow-sm">
